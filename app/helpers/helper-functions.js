...
function formatApplicationCode (request) {
  let appKey = getYarValue(request, 'app-key')
  if (appKey && appKey !== null) {
    return appKey
  }
  const uuid = Uuid.v4()
  appKey = `SGS001-${uuid.substr(0, 3)}-${uuid.substr(3, 3)}`.toUpperCase()
  setYarValue(request, 'app-key', appKey)
  return appKey
}
